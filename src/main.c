...
static bool ram_integrity_check(void)
{
    for (uint8_t i = 0; i < 8; i++)
        ...
        // if (memory_read_dword(MAGIC_ADDR+4*i) != reverse_dword(MAGIC))  // load bytes in order (will erase SRAM)
            return false;
    return true;
}
...
static void ram_initialize(void)
{
    for (uint8_t i = 0; i < 8; i++)
        ...
        // memory_write_dword(MAGIC_ADDR+4*i, reverse_dword(MAGIC));  // load bytes in order (will erase SRAM)
    settings_init();
